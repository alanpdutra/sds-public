...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h4 class="mb-0"><i class="bi bi-people"></i> Autores</h4>
  <a href="{{ url('/authors/create') }}" class="btn btn-primary"><i class="bi bi-plus"></i> Novo</a>
  </div>
...
<div class="card">
  <div class="card-body">
    <div class="row g-2 mb-3">
      <div class="col-md-6">
        <input id="filterNome" class="form-control" placeholder="Filtrar por nome">
      </div>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const load = async () => {
      ...
      try {
        const res = await window.api.listAuthorsCrud({ page, nome: nome.value });
        const data = res && res.ok ? (res.data || []) : [];
        tbody.innerHTML = '';
        data.forEach(a => {
          const tr = document.createElement('tr');
          tr.innerHTML = `
            <td><strong>#${a.CodAu}</strong></td>
            <td>${a.Nome}</td>
            <td>
              <div class="btn-group">
                <a href="/authors/${a.CodAu}/edit" class="btn btn-sm btn-outline-warning"><i class="bi bi-pencil"></i> Editar</a>
                <button class="btn btn-sm btn-outline-danger js-delete" data-id="${a.CodAu}" data-name="${a.Nome}"><i class="bi bi-trash"></i> Excluir</button>
              </div>
            </td>`;
          ...
          delBtn.addEventListener('click', async () => {
            ...
            const ok = await window.UIFx?.confirmDeletion({
              ...
              message: `Tem certeza que deseja excluir o autor "${name}"?`,
            });
            if (!ok) return;
            try {
              window.formUtils.showOverlay();
              const resDel = await window.api.deleteAuthor(id);
